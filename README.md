...
<html lang="en">
 <head>
  ...
  <title>Who Created My Salt? Game</title>
 ...
 <body>
  <div class="game-wrapper">
   <div class="game-container">
    <div class="header">
     <h1 class="game-title" id="gameTitle">Fun with Acids and Bases</h1>
     <p class="subtitle" id="subtitle">Acids, Bases, and Salts</p>
    </div>
    <div id="playerSetupSection" class="player-input-section">
     ...
     <input
       ...
       style="margin-bottom: 25px; width: 100%; max-width: 400px;"
     >
     <div style="margin-top: 30px;">
      <h3 style="margin-bottom: 20px;">🐾 Choose Your Animal</h3>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; max-width: 600px; margin: 0 auto;">
       <div>
        <label style="display: block; margin-bottom: 10px; font-weight: bold; color: #667eea;">Animal Color:</label>
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;">
          <button class="color-btn active" data-color="#FFB347" onclick="selectAnimalColor('#FFB347')" style="background: #FFB347;"></button>
          <button class="color-btn" data-color="#87CEEB" onclick="selectAnimalColor('#87CEEB')" style="background: #87CEEB;"></button>
          <button class="color-btn" data-color="#FFB6C1" onclick="selectAnimalColor('#FFB6C1')" style="background: #FFB6C1;"></button>
          <button class="color-btn" data-color="#90EE90" onclick="selectAnimalColor('#90EE90')" style="background: #90EE90;"></button>
          <button class="color-btn" data-color="#DDA0DD" onclick="selectAnimalColor('#DDA0DD')" style="background: #DDA0DD;"></button>
          <button class="color-btn" data-color="#F0E68C" onclick="selectAnimalColor('#F0E68C')" style="background: #F0E68C;"></button>
        </div>
       </div>
       <div>
        <label style="display: block; margin-bottom: 10px; font-weight: bold; color: #667eea;">Animal Type:</label>
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center;">
          <button class="hair-btn active" onclick="selectAnimalType('cat')" style="font-size: 32px;">🐱</button>
          <button class="hair-btn" onclick="selectAnimalType('dog')" style="font-size: 32px;">🐶</button>
          <button class="hair-btn" onclick="selectAnimalType('rabbit')" style="font-size: 32px;">🐰</button>
          <button class="hair-btn" onclick="selectAnimalType('bear')" style="font-size: 32px;">🐻</button>
        </div>
       </div>
      </div>
      <div style="margin-top: 30px; padding: 20px; background: #f8f9fa; border-radius: 15px;">
       <p style="margin-bottom: 15px; font-weight: bold;">Preview:</p>
     ...
     <button class="btn btn-primary" onclick="startGame()" style="margin-top: 30px; font-size: 18px; padding: 18px 40px;">🎮 Start Game</button>
    </div>
    <div id="gameContent" class="hidden">
     <div class="mode-selection">
      <div class="mode-card" id="mode1Card" onclick="selectMode(1)">
       <div class="mode-title" id="mode1Title">
        Mode 1: Find Salt Origins
       </div>
       <div class="mode-description">
        Given a salt, drag and match the acid and base (10 questions)
       </div>
      </div>
      <div class="mode-card" id="mode2Card" onclick="selectMode(2)">
       <div class="mode-title" id="mode2Title">
        Mode 2: Predict the Salt
       </div>
       <div class="mode-description">
        Given an acid and a base, predict the salt formed (10 questions)
       </div>
      </div>
     </div>
  ...
  <script>
    const defaultConfig = {
      game_title: 'Fun with Acids and Bases',
      subtitle: 'Acids, Bases, and Salts',
      mode1_button: 'Who Created Me?',
      mode2_button: 'Predict the Salt'
    };

    const questions = [
      { mode: 1, salt: 'NaCl', saltName: 'Sodium chloride', acid: 'HCl', base: 'NaOH', decoys: ['H₂SO₄', 'KOH', 'Ca(OH)₂', 'HNO₃', 'NH₃', 'CH₃COOH'] },
      { mode: 1, salt: 'KNO₃', saltName: 'Potassium nitrate', acid: 'HNO₃', base: 'KOH', decoys: ['HCl', 'NaOH', 'H₂SO₄', 'Ca(OH)₂', 'NH₃', 'H₃PO₄'] },
      { mode: 1, salt: 'CaSO₄', saltName: 'Calcium sulfate', acid: 'H₂SO₄', base: 'Ca(OH)₂', decoys: ['HCl', 'NaOH', 'HNO₃', 'KOH', 'Mg(OH)₂', 'HBr'] },
      { mode: 1, salt: 'MgCl₂', saltName: 'Magnesium chloride', acid: 'HCl', base: 'Mg(OH)₂', decoys: ['H₂SO₄', 'Ca(OH)₂', 'HNO₃', 'NaOH', 'KOH', 'H₃PO₄'] },
      { mode: 1, salt: 'K₂SO₄', saltName: 'Potassium sulfate', acid: 'H₂SO₄', base: 'KOH', decoys: ['HCl', 'NaOH', 'HNO₃', 'Ca(OH)₂', 'NH₃', 'CH₃COOH'] },
      { mode: 1, salt: 'NH₄Cl', saltName: 'Ammonium chloride', acid: 'HCl', base: 'NH₃', decoys: ['H₂SO₄', 'NaOH', 'HNO₃', 'KOH', 'Ca(OH)₂', 'CH₃COOH'] },
      { mode: 1, salt: 'CH₃COONa', saltName: 'Sodium acetate', acid: 'CH₃COOH', base: 'NaOH', decoys: ['HCl', 'KOH', 'H₂SO₄', 'Ca(OH)₂', 'HNO₃', 'NH₃'] },
      { mode: 1, salt: 'Na₂SO₄', saltName: 'Sodium sulfate', acid: 'H₂SO₄', base: 'NaOH', decoys: ['HCl', 'KOH', 'HNO₃', 'Ca(OH)₂', 'NH₃', 'Mg(OH)₂'] },
      { mode: 1, salt: 'Ca(NO₃)₂', saltName: 'Calcium nitrate', acid: 'HNO₃', base: 'Ca(OH)₂', decoys: ['HCl', 'NaOH', 'H₂SO₄', 'KOH', 'NH₃', 'CH₃COOH'] },
      { mode: 1, salt: 'MgSO₄', saltName: 'Magnesium sulfate', acid: 'H₂SO₄', base: 'Mg(OH)₂', decoys: ['HCl', 'NaOH', 'HNO₃', 'KOH', 'Ca(OH)₂', 'NH₃'] },
    ...
    async function startGame() {
      ...
      if (!playerName) {
        showInlineMessage('Please enter a player name', 'error');
        return;
      }
    ...
    function loadQuestion() {
      ...
      document.getElementById('questionNumber').textContent = `Question ${currentQuestion + 1}/${filteredQuestions.length}`;
    ...
    function checkAnswerMode1() {
      ...
      if (isCorrect) {
        ...
        resultDiv.textContent = '✅ Correct! +10 points';
        acidZone.classList.add('correct');
        baseZone.classList.add('correct');
      } else {
        resultDiv.className = 'result-message incorrect';
        resultDiv.textContent = `❌ Incorrect. The correct answer is ${question.acid} + ${question.base}`;
    ...
    function checkAnswerMode2() {
      ...
      if (isCorrect) {
        ...
        resultDiv.className = 'result-message incorrect';
        resultDiv.textContent = `❌ Incorrect. The correct answer is ${question.salt}`;
      }
    ...
    async function endGame() {
      const modeText = currentMode === 1 ? 'Mode 1' : 'Mode 2';
      
      if (allScores.length >= 999) {
        showInlineMessage('Cannot save score: storage is full (999 records)', 'error');
      } else {
        document.getElementById('gameArea').innerHTML = `
          <div style="text-align: center; padding: 40px;">
            <div class="loading">Saving score...</div>
          </div>
        `;
        ...
        if (!result.isOk) {
          showInlineMessage('An error occurred while saving the score', 'error');
        }
      ...
      document.getElementById('gameArea').innerHTML = `
        <div style="text-align: center; padding: 40px;">
          <h2 style="font-size: 32px; color: #667eea; margin-bottom: 20px;">🎉 Game Over!</h2>
          <p style="font-size: 24px; margin-bottom: 30px;">Your score: <strong>${score}</strong> points</p>
          ${competitionSummary}
    ...
    function generateCompetitionSummary() {
      ...
      if (sameModeScores.length === 0) {
        return '<p style="font-size: 16px; color: #666; margin-bottom: 20px;">You are the first player in this mode!</p>';
      }
      ...
      if (playerRank === 1) {
        rankEmoji = '🥇';
        rankColor = '#FFD700';
        rankMessage = 'Amazing! You are ranked #1';
      } else if (playerRank === 2) {
        rankEmoji = '🥈';
        rankColor = '#C0C0C0';
        rankMessage = 'Great job! You are ranked #2';
      } else if (playerRank === 3) {
        rankEmoji = '🥉';
        rankColor = '#CD7F32';
        rankMessage = 'Well done! You are ranked #3';
      } else if (playerRank <= 5) {
        rankEmoji = '⭐';
        rankColor = '#667eea';
        rankMessage = `Nice work! You are in the Top 5 (Rank ${playerRank})`;
      } else if (playerRank > 0) {
        rankEmoji = '🏅';
        rankColor = '#666';
        rankMessage = `You are ranked ${playerRank} out of ${totalPlayers} players`;
      } else {
        rankEmoji = '🏅';
        rankColor = '#666';
        rankMessage = `There are ${totalPlayers} players in this mode`;
      }

      let comparisonText = '';
      if (score > averageScore) {
        const diff = score - averageScore;
        comparisonText = `<p style="color: #28a745; font-size: 16px; margin: 10px 0;">📈 Above average by ${diff} points!</p>`;
      } else if (score < averageScore) {
        const diff = averageScore - score;
        comparisonText = `<p style="color: #ff9800; font-size: 16px; margin: 10px 0;">📊 Below average by ${diff} points</p>`;
      } else {
        comparisonText = `<p style="color: #667eea; font-size: 16px; margin: 10px 0;">📊 Exactly the average score!</p>`;
      }
        ...
        <div style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); padding: 25px; border-radius: 15px; margin-bottom: 30px; box-shadow: 0 5px 15px rgba(0,0,0,0.1);">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px; margin-top: 20px;">
            <div style="background: white; padding: 15px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
              <div style="font-size: 14px; color: #666; margin-bottom: 5px;">Number of players</div>
              <div style="font-size: 24px; font-weight: bold; color: #667eea;">${totalPlayers}</div>
            </div>
            
            <div style="background: white; padding: 15px; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
              <div style="font-size: 14px; color: #666; margin-bottom: 5px;">Highest score</div>
          ...
          ${score === topScore ? '<p style="font-size: 18px; color: #28a745; font-weight: bold; margin-top: 15px;">🎊 You got the highest score!</p>' : ''}
        </div>
      `;
    }
    ...
    function renderLeaderboard() {
      ...
      if (topScores.length === 0) {
        list.innerHTML = '<li class="loading">No scores yet</li>';
        return;
      }
      ...
      list.innerHTML = topScores.map((record, index) => `
        <li class="leaderboard-item">
          ...
          <span class="leaderboard-score">${record.score} points (${record.mode})</span>
        </li>
      `).join('');
    }
